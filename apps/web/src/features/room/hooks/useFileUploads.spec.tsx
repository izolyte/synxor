...
function Harness({
  ...
  initial: QueuedFile[];
  /** Start with no session token; the grant-token button supplies one later. */
  withoutToken?: boolean;
}) {
  const [files, setFiles] = useState<QueuedFile[]>(initial);
  const [token, setToken] = useState<string | undefined>(withoutToken ? undefined : "tok");
  const uploaderFn = useMemo(() => uploader.fn, [uploader]);
  const states = useFileUploads(files, token, "http://api.test", uploaderFn);
  return (
    <>
      {initial.map((f) => (
      ...
      <button onClick={() => uploader.calls.at(-1)?.reject(new TypeError("fetch failed"))}>
        fail-unknown
      </button>
...
suite("useFileUploads", () => {
  ...
  test("holds the queue until a session token arrives, then starts", async () => {
    const uploader = new FakeUploader();
    const screen = renderComponent(
      <Harness uploader={uploader} initial={[queued("a.txt")]} withoutToken />,
    );

    await screen.find({ testId: "a.txt" }).shouldHaveText("none");
    expect(uploader.calls.length).toBe(0);

    await screen.find({ role: "button", name: "grant-token" }).click();
    await screen.find({ testId: "a.txt" }).shouldHaveText("uploading:0");
  });

  test("maps an unexpected failure (not an UploadError) to the generic message", async () => {
    const uploader = new FakeUploader();
    const screen = renderComponent(<Harness uploader={uploader} initial={[queued("a.txt")]} />);
    await screen.find({ testId: "a.txt" }).shouldHaveText("uploading:0");

    await screen.find({ role: "button", name: "fail-unknown" }).click();

    await screen
      .find({ testId: "a.txt" })
      .shouldHaveText("error:Upload failed. Check your connection and try again.");
  });
